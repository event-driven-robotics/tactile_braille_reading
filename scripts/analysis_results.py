...
def main():
    ...
    idc_input_activity_first_epoch_ordered = np.argsort(
        input_activity_first_epoch)
    ...
    ax1.set_title("First epoch", fontsize=20)
    ax1.set_ylabel("Trace", fontsize=18)
    ...
    ax1.legend(sorted_handles, sorted_labels, fontsize=14)
    ...
    ax1.set_xticks(np.arange(0, input_traces_ordered_first_epoch.shape[1], 500))
    ax1.set_xticklabels(
        np.arange(0, input_traces_ordered_first_epoch.shape[1]/1000, 0.5))
    ax2.set_xticks(np.arange(0, input_traces_ordered_first_epoch.shape[1], 500))
    ax2.set_xticklabels(
        np.arange(0, input_traces_ordered_first_epoch.shape[1]/1000, 0.5))
    ...
    ax.plot(input_traces_ordered_first_epoch.transpose(),
            label=idc_input_activity_first_epoch_ordered[neurons_of_interest])
    ax.set_title("First epoch", fontsize=20)
    ...
    for i in range(len(sorted_labels)):
        sorted_labels[i] = "Neuron ID: " + sorted_labels[i]
    # Reassign the legend with sorted labels and corresponding handles
    ax.legend(sorted_handles, sorted_labels, fontsize=14)
    ...
    ax.set_xticks(np.arange(0, input_traces_ordered_first_epoch.shape[1], 500))
    ax.set_xticklabels(
        np.arange(0, input_traces_ordered_first_epoch.shape[1]/1000, 0.5))
    ...
    # Adjust the secondary axis
    ylabel = ax2.set_ylabel("Neuron", fontsize=18)
    ...
    ax1.set_title("First epoch", fontsize=20)
    ax1.set_ylabel("Traces", fontsize=18)
    ...
    ax1.set_title("Last epoch", fontsize=20)
    ax1.set_ylabel("Trace", fontsize=18)
    ...
    ax.plot(input_traces_ordered_last_epoch.transpose(),
            label=idc_input_activity_last_epoch_ordered[neurons_of_interest])
    ax.set_title("Last epoch", fontsize=20)
    ax.set_ylabel("Trace", fontsize=18)
    ...
    label = r"$W_{\mathrm{in}}^{(" + \
        f"{idc_max_change_in[0][0]}, {idc_max_change_in[1][0]}" + ")}$"
    ax1.plot(range(1, w_in.shape[0]+1), w_in, label=label)
    label = r"$W_{\mathrm{hidden}}^{(" + \
        f"{idc_max_change_rec[0][0]}, {idc_max_change_rec[1][0]}" + ")}$"
    ax1.plot(range(1, w_in.shape[0]+1), w_rec, label=label)
    label = r"$W_{\mathrm{out}}^{(" + \
        f"{idc_max_change_out[0][0]}, {idc_max_change_out[1][0]}" + ")}$"
    ...
    ax5.set_title("Weight change over batches", fontsize=20)
    ax5.set_ylabel("Weight", fontsize=18)
    ax5.set_xlabel("Batch", fontsize=18)
    ax5.set_xlim(1, w_in.shape[0])
    ax5.tick_params(axis='both', which='major', labelsize=14)
    ax5.legend(loc="center right", fontsize=14)
    ...
    ax2.set_title("Last epoch", fontsize=20)
    ax2.set_ylabel("Weight", fontsize=18)
    ...
    fig.tight_layout()
    fig.savefig(
        "./figures/e_prop_analysis/weights_change_over_batches.pdf", bbox_inches="tight")
    plt.close(fig)
    ...
    label = r"$W_{\mathrm{in}}^{(" + \
        f"{idc_max_change_in[0][0]}, {idc_max_change_in[1][0]}" + ")}$"
    ax.plot(range(1, data_dict_epochs["w_in"].shape[0]+1),
            data_dict_epochs["w_in"], label=label)
    label = r"$W_{\mathrm{hidden}}^{(" + \
        f"{idc_max_change_rec[0][0]}, {idc_max_change_rec[1][0]}" + ")}$"
    ax.plot(range(1, data_dict_epochs["w_in"].shape[0]+1),
            data_dict_epochs["w_rec"], label=label)
    label = r"$W_{\mathrm{out}}^{(" + \
        f"{idc_max_change_out[0][0]}, {idc_max_change_out[1][0]}" + ")}$"
    ax.plot(range(1, data_dict_epochs["w_in"].shape[0]+1),
            data_dict_epochs["w_out"], label=label)
    ax.set_title("Weight change over epochs", fontsize=20)
    ...
    # ax6.set_title("Weight change over epochs", fontsize=20)
    # ax6.set_ylabel("Weight", fontsize=18)
    ...
    fila_name = "test_eprop_5rundef.txt"
    acc_eprop_data = []
    new_data = []  # train, test
    ...
    with open(f"{file_path}/{fila_name}", "r") as f:
        for line in tqdm(f):
            if "Final results" in line:
                acc_eprop_data.append(new_data)
                new_data = []
    ...
    acc_eprop_data = np.array(acc_eprop_data)
    mean_acc_eprop, std_acc_eprop = np.mean(acc_eprop_data, axis=0), np.std(
        acc_eprop_data, axis=0)  # train, test
    
    fila_name = "test_bptt_5_test_200dataset.txt"
    acc_bptt_data = []
    new_data = []  # train, test
    # we need to read the file line by line
    with open(f"{file_path}/{fila_name}", "r") as f:
        for line in tqdm(f):
            ...
            if "Train acc" in line:
                ...
                new_data.append([float(parts[2]), float(parts[5])])
    # Convert the list to a numpy array
    acc_bptt_data = np.array(acc_bptt_data)
    mean_acc_bptt, std_acc_bptt = np.mean(acc_bptt_data, axis=0), np.std(
        acc_bptt_data, axis=0)  # train, test
    
    fig = plt.figure(figsize=(10, 6))
    ax = fig.add_subplot(111)
    ax.plot(range(1, mean_acc_eprop.shape[0]+1), mean_acc_eprop[:, 0], label="E-prop train avg")
    ax.fill_between(range(1, mean_acc_eprop.shape[0]+1),
                    mean_acc_eprop[:, 0] - std_acc_eprop[:, 0], mean_acc_eprop[:, 0] + std_acc_eprop[:, 0], alpha=0.2, label="E-prop train std")
    ax.plot(range(1, mean_acc_eprop.shape[0]+1), mean_acc_eprop[:, 1], label="E-prop test avg")
    ax.fill_between(range(1, mean_acc_eprop.shape[0]+1),
                    mean_acc_eprop[:, 1] - std_acc_eprop[:, 1], mean_acc_eprop[:, 1] + std_acc_eprop[:, 1], alpha=0.2, label="E-prop test std")

    ax.plot(range(1, mean_acc_bptt.shape[0]+1), mean_acc_bptt[:, 0], label="BPTT train avg")
    ax.fill_between(range(1, mean_acc_bptt.shape[0]+1),
                    mean_acc_bptt[:, 0] - std_acc_bptt[:, 0], mean_acc_bptt[:, 0] + std_acc_bptt[:, 0], alpha=0.2, label="BPTT train std")
    ax.plot(range(1, mean_acc_bptt.shape[0]+1), mean_acc_bptt[:, 1], label="BPTT test avg")

    ax.fill_between(range(1, mean_acc_bptt.shape[0]+1),
                    mean_acc_bptt[:, 1] - std_acc_bptt[:, 1], mean_acc_bptt[:, 1] + std_acc_bptt[:, 1], alpha=0.2, label="BPTT test std")
    ax.set_xticks(np.arange(0, mean_acc_eprop.shape[0]+1, 5))
    ax.set_xticklabels(np.arange(0, mean_acc_eprop.shape[0]+1, 5))
    ax.set_ylim(0, 100)
    ax.set_xlim(1, mean_acc_eprop.shape[0])
    ax.set_xlabel("Epoch", fontsize=18)
    ...
    fig.tight_layout()
    fig.savefig("./figures/e_prop_analysis/accuracy_mean_std.pdf",
                bbox_inches="tight")
    plt.close(fig)

    # HERE WE ADD TO THE MAIN FIGURE
    ax7 = fig_final.add_subplot(gs_final[1, 1])
    ax7.fill_between(range(1, mean_acc_eprop.shape[0]+1),
                    mean_acc_eprop[:, 0] - std_acc_eprop[:, 0], mean_acc_eprop[:, 0] + std_acc_eprop[:, 0], alpha=0.2, label="E-prop train std")
    ax7.plot(range(1, mean_acc_eprop.shape[0]+1), mean_acc_eprop[:, 0], label="E-prop train avg")
    ax7.fill_between(range(1, mean_acc_eprop.shape[0]+1),
                    mean_acc_eprop[:, 1] - std_acc_eprop[:, 1], mean_acc_eprop[:, 1] + std_acc_eprop[:, 1], alpha=0.2, label="E-prop test std")
    ax7.plot(range(1, mean_acc_eprop.shape[0]+1), mean_acc_eprop[:, 1], label="E-prop test avg")

    ax7.fill_between(range(1, mean_acc_bptt.shape[0]+1),
                    mean_acc_bptt[:, 0] - std_acc_bptt[:, 0], mean_acc_bptt[:, 0] + std_acc_bptt[:, 0], alpha=0.2, label="BPTT train std")
    ax7.plot(range(1, mean_acc_bptt.shape[0]+1), mean_acc_bptt[:, 0], label="BPTT train avg")
    ax7.fill_between(range(1, mean_acc_bptt.shape[0]+1),
                    mean_acc_bptt[:, 1] - std_acc_bptt[:, 1], mean_acc_bptt[:, 1] + std_acc_bptt[:, 1], alpha=0.2, label="BPTT test std")
    ax7.plot(range(1, mean_acc_bptt.shape[0]+1), mean_acc_bptt[:, 1], label="BPTT test avg")

    ax7.set_xticks(np.arange(0, mean_acc_eprop.shape[0]+1, 5))
    ax7.set_xticklabels(np.arange(0, mean_acc_eprop.shape[0]+1, 5))
    ax7.set_ylim(0, 100)
    ax7.set_xlim(1, mean_acc_eprop.shape[0])
    ax7.set_xlabel("Epoch", fontsize=18)
    ax7.set_ylabel("Accuracy", fontsize=18)
    ax7.set_title("Training", fontsize=20)
    ...
    fig_final.savefig(
        "./figures/e_prop_analysis/training_results.pdf", bbox_inches="tight")
    plt.close(fig_final)
