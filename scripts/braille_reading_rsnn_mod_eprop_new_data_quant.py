...
def run_snn(inputs, layers, trainable, yt = None):
    ...
    if(trainable):
        max_spikes, _ = torch.max(n_spike, dim=2, keepdim=True)
...
def build_and_train(params, ds_train, ds_test, epochs=epochs):
    ...
    layers = []
    if(load_models):
        layers = torch.load('./model/best_model_net_th_2.pt', weights_only=True)
...
if __name__ == '__main__':
    with torch.no_grad():
        ...
        # save the best layer
        torch.save(very_best_layer, './model/best_model_net_th_'+str(threshold)+'_'+str(nb_hidden)+'_'+ str(lr)+'_'+str(tau_trace)+'.pt')
