...
ff_syn_rec = np.zeros((nb_steps, batch_size, nb_ff_neurons))
ff_mem_rec = np.zeros((nb_steps, batch_size, nb_ff_neurons))

ff_mem_rec = np.zeros((nb_steps, batch_size, nb_ff_neurons))
rec_syn_rec = np.zeros((nb_steps, batch_size, nb_rec_neurons))
...
for t in range(nb_steps):
    ...
    ff_spk, ff_syn, ff_rec = feedforward_layer.update(input_activity_t)
    rec_spk, rec_syn, rec_mem = recurrent_layer.update(ff_spk)
    # Store the results
    ff_spk_rec[t] = ff_spk.detach().cpu().numpy()
    ff_syn_rec[t] = ff_syn.detach().cpu().numpy()
    ff_mem_rec[t] = ff_rec.detach().cpu().numpy()
    rec_syn_rec[t] = rec_syn.detach().cpu().numpy()
